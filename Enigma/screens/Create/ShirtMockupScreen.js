...
const ShirtMockupScreen = ({
    ...
    const handleConfirm = () => {
        if (
            selectedColor === null ||
            (selectedSize === null && createType === "ORDER") ||
            !isSelectBack
        ) {
            setShowModal(true);
        } else {
            saveProductName("Áo thun");
            saveProductDescription(
                `Áo thun ${formatColor(selectedColor)}, size ${selectedSize}`
            );
            saveSizeOption(selectedSize);
    ...
    const renderMissingOption = () => {
        if (selectedColor === null && selectedSize === null) {
            return "Bạn chưa chọn Màu sắc và Kích cỡ cho sản phẩm";
        }
        if (selectedColor === null) {
            return "Bạn chưa chọn Màu sắc cho sản phẩm";
        }

        if (selectedSize === null && createType === "ORDER") {
            return "Bạn chưa chọn Kích cỡ cho sản phẩm";
        }

        if (!isSelectBack) {
            return "Vui lòng tuỳ chọn mặt sau của áo";
        }
    };
        ...
        const mockupOptions = [
            {
                title: "Trước",
                image: selectedColor
                    ? mockupUri.tee[selectedColor].front
                    : USER_IMAGE_SOURCE,
            },
            {
                title: "Sau",
                image: selectedColor
                    ? mockupUri.tee[selectedColor].back
                    : USER_IMAGE_SOURCE,
            },
        ];
    ...
    const renderMockupFront = () => {
        ...
        const mockupImageUri = mockupUri.tee[selectedColor].front;
        return (
            <ViewShot ref={viewShotRefFront}>
                <Image source={mockupImageUri} style={styles.image} />
                {isShowImage && (
    ...
    const renderMockupBack = () => {
        // Render cho mặt sau
        const mockupImageUri = mockupUri.tee[selectedColor].back;
        return (
...
export default connect(mapStateToProps, mapDispatchToProps)(ShirtMockupScreen);
